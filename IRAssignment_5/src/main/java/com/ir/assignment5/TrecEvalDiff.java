...
/**
 * Created by ps on 7/19/17.
 */
public class TrecEvalDiff {
    ...
    private void loadResultMap(String result) throws IOException {
        ...
        while((line=br.readLine())!=null){
            ...
            Doc doc = new Doc();
            doc.setDocId(resultArr[2].trim());
            doc.setScore(Double.parseDouble(resultArr[4].trim()));
            doc.setRank(Integer.parseInt(resultArr[3].trim()));
            if(qrelMap.containsKey(resultArr[0].trim()) &&
                    qrelMap.get(resultArr[0].trim()).containsKey(resultArr[2].trim())){
                doc.setRelevance(qrelMap.get(resultArr[0].trim()).get(resultArr[2].trim()));
            }else{
                doc.setRelevance(0);
            }
...
//                qrelMap.get(qrelArr[0]).put(qrelArr[2].trim(), Integer.parseInt(qrelArr[3].trim()));
//            } else{
//                Map<String,Integer> tempMap = new HashMap<String, Integer>();
//                tempMap.put(qrelArr[2], Integer.parseInt(qrelArr[3]));
...
//        br.close();
    ...
    public void generatePrecRecall() {
        for(String t : resultMap.keySet()){
            int relevalntDoc = getTotalRelevantDocs(qrelMap.get(t));
            //System.out.println(relevalntDoc + "rel docs");
    ...
    private void calculatePrecRecall(List<Doc> list, int totalRelevalntDoc) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            tempDoc.setRecall((double)relevanceCount/totalRelevalntDoc);
    ...
    private int getTotalRelevantDocs(Map<String, Integer> strMap) {
        int relCount = 0;
    ...
    public void generateMeasureScores() {
        for (String query : resultMap.keySet()) {
            ...
            int total_number_of_relevant = getTotalRelevantDocs(qrelMap.get(query));
            //System.out.println("tot" + total_number_of_relevant);
            ...
            //System.out.println("avg" + avgPrecision + "rprec"+ rPrecision);
    ...
    private int getRelDocs(List<Doc> docs) {
        int count = 0;
        for (Doc doc : docs) {
